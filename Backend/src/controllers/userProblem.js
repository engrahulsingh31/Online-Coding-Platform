...
const getProblembyid=async(req,res)=>{
  ...
  try{
    ...
    const getProblem = await Problem.findById(id).select('_id title description difficulty tags visiableTestCase startcode referenceSolution ');
...
const getAllproblem =async(req,res)=>{
        const getProblem=await Problem.find({}).select('_id title description difficulty tags visibleTestCases startCode referenceSolution');
...
const problemSolvedbyuser=async(req,res)=>{
    try{
        ...
        res.status(500).send("Srver Error:"+err)
    }
